class GM_Matrix4 {
	...
	static GM_Matrix4 create() {
		return new("GM_Matrix4");
	}

	/// Returns an identity matrix.
	static GM_Matrix4 identity() {
		let ret = GM_Matrix4.create();
		ret.values[0][0] = 1;
		ret.values[1][1] = 1;
		ret.values[2][2] = 1;
		ret.values[3][3] = 1;
	...
	static GM_Matrix4 fromEulerAngles(double yaw, double pitch, double roll) {
		GM_Matrix4 rYaw = GM_Matrix4.identity();
		double sYaw = sin(yaw);
		double cYaw = cos(yaw);
		...
		GM_Matrix4 rPitch = GM_Matrix4.identity();
		...
		GM_Matrix4 rRoll = GM_Matrix4.identity();
		...
		GM_Matrix4 ret = rYaw.multiplyMatrix(rPitch);
		ret = ret.multiplyMatrix(rRoll);
		return ret;
	}

	/// Returns a rotation matrix from an axis and an angle.
	static GM_Matrix4 fromAxisAngle(Vector3 axis, double angle) {
		GM_Matrix4 ret = GM_Matrix4.identity();
		double c = cos(angle);
		double s = sin(angle);
		double x = axis.x;
		double y = axis.y;
	...
	double, double, double rotationToEulerAngles() {
		if (GM_GlobalMaths.closeEnough(values[2][0], -1)) {
			double x = 90;
			double y = 0;
			double z = atan2(values[0][1], values[0][2]);
			return z, x, y;
		}
		else if (GM_GlobalMaths.closeEnough(values[2][0], 1)) {
			double x = -90;
			double y = 0;
			double z = atan2(-values[0][1], -values[0][2]);
			return z, x, y;
	...
	static GM_Matrix4 createTRSEuler(Vector3 translate, double yaw, double pitch, double roll, Vector3 scale) {
		GM_Matrix4 translateMat = GM_Matrix4.identity();
		translateMat.values[0][3] = translate.x;
		translateMat.values[1][3] = translate.y;
		translateMat.values[2][3] = translate.z;
		
		GM_Matrix4 rotateMat = GM_Matrix4.fromEulerAngles(yaw, pitch, roll);
		
		GM_Matrix4 scaleMat = GM_Matrix4.identity();
		scaleMat.values[0][0] = scale.x;
		scaleMat.values[1][1] = scale.y;
		scaleMat.values[2][2] = scale.z;
		
		GM_Matrix4 ret = translateMat.multiplyMatrix(rotateMat);
		ret = ret.multiplyMatrix(scaleMat);
		return ret;
	}
	
	static GM_Matrix4 createTRSAxisAngle(Vector3 translate, Vector3 axis, double angle, Vector3 scale) {
		GM_Matrix4 translateMat = GM_Matrix4.identity();
		translateMat.values[0][3] = translate.x;
		translateMat.values[1][3] = translate.y;
		translateMat.values[2][3] = translate.z;
		
		GM_Matrix4 rotateMat = GM_Matrix4.fromAxisAngle(axis, angle);
	...
	static GM_Matrix4 view(Vector3 camPos, double yaw, double pitch, double roll) {
		...
		// pixel ratio scaling
		GM_Matrix4 scale = GM_Matrix4.identity();
		scale.values[1][1] = pixelRatio;
		// swapping y and z
		GM_Matrix4 swapYZ = GM_Matrix4.create();
		swapYZ.values[0][0] = 1;
		swapYZ.values[1][2] = 1;
		swapYZ.values[2][1] = -1;
		swapYZ.values[3][3] = 1;
		// translation
		GM_Matrix4 translate = GM_Matrix4.identity();
		...
		GM_Matrix4 ret = rot.multiplyMatrix(scale);
	...
	static GM_Matrix4 perspective(double fovy, double aspect, double zNear, double zFar) {
		GM_Matrix4 ret = GM_Matrix4.create();
	...
	static GM_Matrix4 worldToClip(Vector3 viewPos, double yaw, double pitch, double roll, double FOV) {
		double aspect = Screen.getAspectRatio();
		double fovy = GM_GlobalMaths.fovHToY(FOV);
		GM_Matrix4 view = GM_Matrix4.view(viewPos, yaw, pitch, roll);
		// 5 & 65535 are what are used internally, so they're used here for consistency
		GM_Matrix4 perp = GM_Matrix4.perspective(fovy, aspect, 5, 65535);
		GM_Matrix4 worldToClip = perp.multiplyMatrix(view);
		return worldToClip;
	}

	/// Adds two matrices and returns the result.
	GM_Matrix4 addMatrix(GM_Matrix4 other) const {
		GM_Matrix4 ret = GM_Matrix4.create();
	...
	GM_Matrix4 multiplyScalar(double scalar) const {
		GM_Matrix4 ret = GM_Matrix4.create();
	...
	GM_Matrix4 multiplyMatrix(GM_Matrix4 other) const {
		GM_Matrix4 ret = GM_Matrix4.create();
	...
	Vector3 multiplyVector3(Vector3 vec, GM_VectorType type = GM_Vector_Position, bool divideW = true) const {
		let vecW = (type == GM_Vector_Position) ? 1.0 : 0.0;
