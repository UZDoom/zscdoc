...
class GM_Matrix {
	...
	GM_Matrix init(int columns, int rows) {
		if (columns <= 0 || rows <= 0) {
	...
	static GM_Matrix create(int columns, int rows) {
		return new("GM_Matrix").init(columns, rows);
	}

	/// Returns an identity matrix.
	static GM_Matrix identity(int dimension) {
		GM_Matrix ret = GM_Matrix.create(dimension, dimension);
	...
	static GM_Matrix fromEulerAngles(double yaw, double pitch, double roll) {
		GM_Matrix rYaw = GM_Matrix.identity(4);
		double sYaw = sin(yaw);
		double cYaw = cos(yaw);
		rYaw.set(0, 0,  cYaw);
		rYaw.set(0, 1, -sYaw);
		...
		GM_Matrix rPitch = GM_Matrix.identity(4);
		...
		GM_Matrix rRoll = GM_Matrix.identity(4);
		...
		GM_Matrix ret = rYaw.multiplyMatrix(rPitch);
		ret = ret.multiplyMatrix(rRoll);
		return ret;
	}

	/// Returns a rotation matrix from an axis and an angle.
	static GM_Matrix fromAxisAngle(Vector3 axis, double angle) {
		GM_Matrix ret = GM_Matrix.identity(4);
		double c = cos(angle);
		double s = sin(angle);
		double x = axis.x;
		double y = axis.y;
	...
	double, double, double rotationToEulerAngles() {
		if (GM_GlobalMaths.closeEnough(get(2, 0), -1)) {
			double x = 90;
			double y = 0;
			double z = atan2(get(0, 1), get(0, 2));
			return z, x, y;
		}
		else if (GM_GlobalMaths.closeEnough(get(2, 0), 1)) {
			double x = -90;
			double y = 0;
			double z = atan2(-get(0, 1), -get(0, 2));
			return z, x, y;
	...
	static GM_Matrix createTRSEuler(Vector3 translate, double yaw, double pitch, double roll, Vector3 scale) {
		GM_Matrix translateMat = GM_Matrix.identity(4);
		translateMat.set(0, 3, translate.x);
		translateMat.set(1, 3, translate.y);
		translateMat.set(2, 3, translate.z);
		
		GM_Matrix rotateMat = GM_Matrix.fromEulerAngles(yaw, pitch, roll);
		
		GM_Matrix scaleMat = GM_Matrix.identity(4);
		scaleMat.set(0, 0, scale.x);
		scaleMat.set(1, 1, scale.y);
		scaleMat.set(2, 2, scale.z);
		
		GM_Matrix ret = translateMat.multiplyMatrix(rotateMat);
		ret = ret.multiplyMatrix(scaleMat);
		return ret;
	}
	
	static GM_Matrix createTRSAxisAngle(Vector3 translate, Vector3 axis, double angle, Vector3 scale) {
		GM_Matrix translateMat = GM_Matrix.identity(4);
		translateMat.set(0, 3, translate.x);
		translateMat.set(1, 3, translate.y);
		translateMat.set(2, 3, translate.z);
		
		GM_Matrix rotateMat = GM_Matrix.fromAxisAngle(axis, angle);
	...
	static GM_Matrix view(Vector3 camPos, double yaw, double pitch, double roll) {
		...
		// rotations
		GM_Matrix rotR = GM_Matrix.fromAxisAngle((0, 0, 1), roll);
		GM_Matrix rotP = GM_Matrix.fromAxisAngle((1, 0, 0), adjustedPitch);
		GM_Matrix rotY = GM_Matrix.fromAxisAngle((0, -1, 0), adjustedYaw);
		// pixel ratio scaling
		GM_Matrix scale = GM_Matrix.identity(4);
		scale.set(1, 1, pixelRatio);
		// swapping y and z
		GM_Matrix swapYZ = GM_Matrix.create(4, 4);
		swapYZ.set(0, 0, 1);
		swapYZ.set(1, 2, 1);
		swapYZ.set(2, 1, -1);
		swapYZ.set(3, 3, 1);
		// translation
		GM_Matrix translate = GM_Matrix.identity(4);
		translate.set(0, 3, -camPos.x);
		translate.set(1, 3, -camPos.y);
		translate.set(2, 3, -camPos.z);

		// concatenate them all to get a final matrix
		GM_Matrix ret = rotR.multiplyMatrix(rotP);
	...
	static GM_Matrix perspective(double fovy, double aspect, double zNear, double zFar) {
		GM_Matrix ret = GM_Matrix.create(4, 4);
	...
	static GM_Matrix worldToClip(Vector3 viewPos, double yaw, double pitch, double roll, double FOV) {
		double aspect = Screen.getAspectRatio();
		double fovy = GM_GlobalMaths.fovHToY(FOV);
		GM_Matrix view = GM_Matrix.view(viewPos, yaw, pitch, roll);
		// 5 & 65535 are what are used internally, so they're used here for consistency
		GM_Matrix perp = GM_Matrix.perspective(fovy, aspect, 5, 65535);
		GM_Matrix worldToClip = perp.multiplyMatrix(view);
		return worldToClip;
	}
	...
	GM_Matrix multiplyScalar(double scalar) const {
		GM_Matrix ret = GM_Matrix.create(rows, columns);
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < columns; col++) {
	...
	GM_Matrix multiplyVector2(Vector2 vec, GM_VectorType type = GM_Vector_Position) const {
		GM_Matrix vec2Matrix = GM_Matrix.create(1, 3);
		vec2Matrix.set(0, 0, vec.x);
		vec2Matrix.set(1, 0, vec.y);
		if (type == GM_Vector_Position)       vec2Matrix.set(2, 0, 1);
		else if (type == GM_Vector_Direction) vec2Matrix.set(2, 0, 0);
	...
	GM_Matrix multiplyVector3(Vector3 vec, GM_VectorType type = GM_Vector_Position) const {
		GM_Matrix vec3Matrix = GM_Matrix.create(1, 4);
		vec3Matrix.set(0, 0, vec.x);
		vec3Matrix.set(1, 0, vec.y);
		vec3Matrix.set(2, 0, vec.z);
		if (type == GM_Vector_Position)       vec3Matrix.set(3, 0, 1);
		else if (type == GM_Vector_Direction) vec3Matrix.set(3, 0, 0);
